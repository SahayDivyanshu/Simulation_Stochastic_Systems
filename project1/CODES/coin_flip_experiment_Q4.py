...
N_flips = 200
p = 0.80 # proving the probability of 0.5 to each
np.random.seed(123) 
...
for i in range(toss.shape[0]):
    if (toss[i] > p):
        toss[i] = 0
        if(head_count > longest_run):
            longest_run = head_count # calculating longest run
        head_count = 0
    else:
        toss[i] = 1
        head_count = head_count + 1 # incrementing head count 
         
print("longest run of heads: ", longest_run)   
print("Total Head:", sum(toss)) 
print(toss) # outcome of 50 tosses
x = range(1, N_flips+1)
plt.hist(toss,2) #plotting histogram
plt.show()
